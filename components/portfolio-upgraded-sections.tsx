...
// CreativeAbout
// -----------------------------
export function CreativeAbout({ aboutRef, skills }: { aboutRef?: React.RefObject<HTMLElement>; skills: any[] }) {
  const ref = aboutRef || useRef<HTMLDivElement | null>(null)
  const inView = useInView(ref, { once: true, margin: "-100px 0px -50% 0px" })
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="absolute -top-20 -left-20 w-72 h-72 bg-primary/20 blur-[100px] rounded-full pointer-events-none"
        animate={{ x: [0, 30, 0], y: [0, -30, 0] }}
        transition={{ duration: 12, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute top-10 right-0 w-60 h-60 bg-secondary/20 blur-[100px] rounded-full pointer-events-none"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-start">
        ...
        {/* LEFT: Enhanced text with highlights */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={inView ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 1, delay: 0.3 }}
          className="text-lg sm:text-xl md:text-2xl font-light text-white/80 leading-relaxed space-y-6"
          style={{ fontFamily: "var(--font-nunito)" }}
        >
          <p>
            I'm a{" "}
            <span className="text-secondary font-semibold">2nd-year Computer Science student</span>{" "}
            at <span className="text-secondary font-semibold">VIT Vellore</span> who loves building{" "}
            <span className="text-primary font-semibold">clean, performant apps</span> and turning
            ideas into real, usable products.
          </p>

          <p>
            I work across{" "}
            <span className="text-secondary font-semibold">full-stack web</span> and{" "}
            <span className="text-primary font-semibold">iOS development</span>, focusing on
            impactful projects like AI tools, startups, and scalable platforms.
          </p>
          ...
          <motion.div
            ...
            className="p-5 rounded-xl border border-white/10 bg-white/5 backdrop-blur-lg shadow-lg shadow-primary/10 hover:border-primary/40 hover:bg-white/[0.08] transition-all duration-300"
          >
            <p className="text-white/90">
              Currently building:  
              <span className="text-secondary ml-2 font-semibold">AI-powered apps</span>  
              +  
              <span className="text-primary ml-2 font-semibold">scalable web systems</span>.
            </p>
          </motion.div>
        </motion.div>

        {/* RIGHT: Skills Grid — modern + glow hover */}
        <motion.div
        ...
        >
          {skills.map((skill, i) => (
            <motion.div
              ...
              className="p-5 rounded-xl border border-white/10 bg-white/[0.04] backdrop-blur-lg text-center flex flex-col items-center gap-2 hover:border-primary/40 hover:bg-white/[0.08] transition-all duration-300 relative overflow-hidden group"
            >
              {/* Glow on hover */}
              <motion.div
                className="absolute inset-0 bg-gradient-to-br from-primary/10 to-secondary/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-xl"
              />

              <div className="z-10 text-white/70 group-hover:text-white transition-colors">
                {skill.icon}
              </div>
              <p className="text-sm text-white/80 group-hover:text-white z-10 transition-colors font-light">
                {skill.name}
              </p>
            </motion.div>
          ))}
      ...
      <motion.div
        ...
        className="h-[2px] bg-gradient-to-r from-primary/60 to-secondary/60 mt-20 rounded-full"
      />
    </motion.section>
  )
}
